/* TODO: add proper comments and javadoc */
...
import Normal2DFunction;
import Normal3DFunction;
import Parametric2DFunction;
import Parametric3DFunction;
import PolarFunction;
...
public final class PlotEngine { 
	public static final string FUNCTION_STYLES[] = {"points", "linespoints", "filledcurves", "lines", "vectors", "pm3d"};
	...
	public PlotEngine() throws IOException { 
		...
		pout = new BufferedReader(new InputStreamReader(plotter.getInputStream()));
		perr = new BufferedReader(new InputStreamReader(plotter.getErrorStream()));
	...
	public void plot(final Parametric3DFunction... funcs) {
		...
		for (Parametric3DFunction... funcs) {
			command.append(func.getXFunction());
			command.append(',');
			command.append(func.getYFunction());
			command.append(',');
	...
	public void setTitle(final String title) {
		final String command = formatter.format("set title %s", title).toString();
	...
	public void setFunctionStyle(final String style) {
		if (!Arrays.asList(FUNCTION_STYLES).contains(style))
			throw new IllegalArgumentException("\"" + style + "\"" + " is not a valid function style.");

		final String command = formatter.format("set style function %s", style).toString();
		sendCommand(command);
	}
	...
	public void setXRange(final double xMin, final double xMax) {
		if (!validDouble(xMin) || !validDouble(xMax))
			throw new IllegalArgumentException("xMin and xMax must be rational numbers.");

		final String command = formatter.format("set xrange [%f:%f]", min(xMin, xMax), max(xMin, xMax)).toString();
	...
	public void setYRange(final double yMin, final double yMax) {
		if (!validDouble(yMin) || !validDouble(yMax))
			throw new IllegalArgumentException("yMin and yMax must be rational numbers.");

		final String command = formatter.format("set yrange [%f:%f]", min(yMin, yMax), max(yMin, yMax)).toString();
	...
	public void setZRange(final double zMin, final double zMax) {
		if (!validDouble(zMin) || !validDouble(zMax))
			throw new IllegalArgumentException("zMin and zMax must be rational numbers.");

		final String command = formatter.format("set zrange [%f:%f]", min(zMin, zMax), max(zMin, zMax)).toString();
	...
	public void setURange(final double uMin, final double uMax) {
		if (!validDouble(uMin) || !validDouble(uMax))
			throw new IllegalArgumentException("uMin and uMax must be rational numbers.");

		final String command = formatter.format("set urange [%f:%f]", min(uMin, uMax), max(uMin, uMax)).toString();
	...
	public void setVRange(final double vMin, final double vMax) {
		if (!validDouble(vMin) || !validDouble(vMax))
			throw new IllegalArgumentException("vMin and vMax must be rational numbers.");

		final String command = formatter.format("set vrange [%f:%f]", min(vMin, vMax), max(vMin, vMax)).toString();
	...
	public void setTRange(final double tMin, final double tMax) {
		final String command = formatter.format("set trange [%f:%f]", min(tMin, tMax), max(tMin, tMax)).toString();
	...
	public void setRRange(final double rMin, final double rMax) {
		final String command = formatter.format("set rrange [%f:%f]", min(rMin, rMax), max(rMin, rMax)).toString();
